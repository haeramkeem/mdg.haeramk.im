...
- 가장 사용 빈도가 낮은 놈을 방출시키는 [[Cache Replacement Policy (memory)|cache replacement policy]] 이다.
- 자주 사용된 놈은 또 사용될 것이라는 [[Spacial Locality (memory)|공간 지역성]] 에 기반한 방법인 셈.
- Heap 을 이용하여 구현한다
    ...
    - 따라서 [[Least Recently Used, LRU (memory)|LRU]] 에 비해서는 실제 작동시에 시간복잡도가 떨어진다.
