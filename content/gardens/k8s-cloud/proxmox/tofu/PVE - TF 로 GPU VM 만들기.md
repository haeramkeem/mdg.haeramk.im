...
> [!danger] 조심할 것 (중요)
> - 일반 VM 의 메모리 할당량은 실제로 설치된 물리 메모리 크기보다 같거나 좀 커도 된다. 어차피 KVM 이 실제로 VM 에서 사용하는 양만을 메모리에 올려놓기 때문.
> - 근데 GPU VM 의 경우에는 그러면 ==절대== 안된다. VFIO 를 쓰면 메모리를 pinning 하기 때문에, 물리 메모리의 양보다 충분히 작게 설정해야 한다.
> - 그래서 VM 을 생성한 뒤에 PVE 대시보드에서 VM 메모리 사용량을 확인하면 항상 100%로 찍히는 것을 볼 수 있다. VM 메모리가 host 메모리에 pinning 되어있기 때문에 host 메모리 사용량 기준으로는 100%인 셈.
> ![[Pasted image 20260808003753.png]]
