...
- [[Domain Name System, DNS (Network)|DNS]] 는 기본적으로 plain text 기반이고, 따라서 개인의 privacy 가 노출될 염러가 있다.
...
- 그래서 DNS 를 기존의 [[Transport Layer Security, TLS (L6 Presentation Layer)|TLS]] 와 같은 encryption protocol 위에 올리는 아이디어가 나오게 된 것.
...
- 853/[[Transmission Control Protocol, TCP (L4 Tranport Layer)|TCP]] 를 사용한다.

### DNS-over-HTTPS (DoH)

- [RFC8484](https://datatracker.ietf.org/doc/html/rfc8484) 로 제안된 것으로, DNS 패킷을 [[HTTP with TLS, HTTPS (L7 Application Layer)|HTTPS]] 로 감싸는 방법이다.
- 기존 HTTPS 와 동일한 443/[[Transmission Control Protocol, TCP (L4 Tranport Layer)|TCP]] 를 사용한다.
