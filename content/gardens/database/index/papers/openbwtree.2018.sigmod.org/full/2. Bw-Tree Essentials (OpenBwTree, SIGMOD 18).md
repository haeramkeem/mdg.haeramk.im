...
- 따라서, delta node append 를 하는 것은 [[Compare and Swap, CAS|CAS]] 로 수행될 수 있다. 좀 더 자세히 보면 아래와 같다.
...
- 이 과정에서 [[Compare and Swap, CAS|CAS]] 가 사용되는 것이다. 유효성 검사와 새로운 address setting 이 atomic 하게 이루어져야 하기 때문.
