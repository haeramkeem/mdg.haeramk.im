...
> [!tip] Heading numbering policy
> - [[#2. FastLanes|Section 2]] 에서의 H4 (`x.x.x.` 로 넘버링된 section) 들은 주인장이 임의로 붙인거고 원본 논문에는 그런 section 은 없습니다.
...
- 따라서 본 논문에서는 *FastLanes-RLE* 라는 것을 제시한다.
	- 근데 뒤에 보면 알겠지만 [[Run Length Encoding, RLE (Encoding)|RLE]] 는 아니다. run 을 활용하기는 하지만 length 로 꼼지락대지는 않기 때문.
