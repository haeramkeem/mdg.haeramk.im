...
- 또 다른 아이디어는 *Four-cursors* 라는 것으로, 저자 아짐씨 본인의 [석사 졸업논문](https://homepages.cwi.nl/~boncz/msc/2020-AzimAfroozeh.pdf) 에 제시한 것이다.
	...
	- 근데 SIMD vectorize 할 수는 없고, 대신 [[Instruction Level Parallelism, ILP (Arch)|ILP]] 로 처리할 수 있다고 한다.
		- SIMD 의 이점을 볼 수는 없는 이유는 값들이 연속된 메모리 공간에 있지 않기 때문인 것으로 보인다.
		- 그래서 SIMD 는 사용할 수 없지만 data dependency 는 해결되기 때문에 [[Instruction Level Parallelism, ILP (Arch)|ILP]] 로는 최적화가 가능할 것이라고 하는 듯.
- 그래서 전체적으로 정리해보면:
...
- 결과는 위와 같다.
	- 일단 Horizontal 은 결과가 일정한데, 이건 이놈이 SSE 에서만 제공하는 `SHIFT` 를 사용하여 다른 architecture 에서는 사용하지 못해 그렇게 그려놓은 것이다.
