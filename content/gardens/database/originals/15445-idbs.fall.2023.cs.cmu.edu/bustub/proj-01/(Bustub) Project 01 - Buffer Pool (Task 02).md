...
- 다만, queue element type 이 `std::optional` 이기 때문에 여기서 좀 애를 먹긴 했다.
	...
	- 그래서 뭐 `DiskRequest` 에 [[Copy Constructor (C++ Class)|copy constructor]] 도 구현해 보고 별 짓을 다 해봤지만
	- 결론적으로 말하면 `DiskRequest` 의 member 인 `callback_` 이 `std::promise` 여서 [[Copy Constructor (C++ Class)|copy constructor]] 나 [[Copy Assignment Operator (C++ Class)|copy assignment operator]] 를 지원하지 않아 [[Move Ownership (C++)|move]] 를 해줌으로써 해결했다.
...
- [[Optional Object (C++ Optional)]]
- [["bugprone-unchecked-optional-access" Error (C++ Optional)]]
- [[Creating Optional Object (C++ Optional)]]
- [[Promise, Future (C++ Async)]]
- [[Constructor (C++ Class)]]
- [[Copy Assignment Operator (C++ Class)]]
- [[Copy Constructor (C++ Class)]]
- [[Destructor (C++ Class)]]
- [[Move Constructor (C++ Class)]]