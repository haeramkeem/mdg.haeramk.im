...
- 이 thread 를 생성하는 것은 OS-independent 한 API 인 POSIX 에 정의되어 있어서 여기에의 thread API ([[C - 세상 간단한 POSIX thread (pthread) 예시|Pthread]]) 를 불러서 thread 기능을 사용해주면 된다.
...
- Run queue 는 [[Red-black Tree, RB Tree (Index)|RBTree]] 로 관리된다.
