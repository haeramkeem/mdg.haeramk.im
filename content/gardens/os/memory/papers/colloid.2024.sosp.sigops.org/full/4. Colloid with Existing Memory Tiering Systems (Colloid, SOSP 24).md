...
- MEMTIS 는 HeMem 과 유사하지만, 다음의 차이점이 있다고 한다:
	...
	4. 마지막으로 MEMTIS 에서는 hugepage 를 이용한다: 특정한 heuristic 에 따라, background thread 가 page 들을 hugepage 로 합치고 `kmigraterd` 가 (hugepage 를 사용하는 것이 효과적이지 못할 경우에) hugepage 를 다시 page 로 분리한다.
