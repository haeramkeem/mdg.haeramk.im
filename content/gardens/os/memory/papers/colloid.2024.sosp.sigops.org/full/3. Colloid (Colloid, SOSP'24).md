...
- 일단 [[#3.1. Access Latency is the Key|Section 3.1]] 에서는 *Colloid* 의 유일한 원칙인 "Balancing latency" 와 이것을 어떻게 구현할 수 있을지 (즉, 어떻게 latency 를 측정할 수 있을지) 에 대해 알아보고, [[#3.2. Colloid Page Placement Algorithm|Section 3.2]] 에서는 이렇게 측정된 latency 를 이용해서 어떻게 page placement 을 할 수 있을지 알아본다.
...
- 이러한 architecture 는 대부분의 tiered memory system 에서 흔하게 사용하는 방식이다:
	...
	- 그리고 alternate tier 는 CXL 이나 remote socket, [[High Bandwidth Memory, HBM (Arch)|HBM]] 등으로 구성될 수 있다.
- *Colloid* 는 fixed time period 인 *Quantum* 마다 동일한 작업을 하는 것을 기준으로 design 되었다.
	- 하지만 [[4. Colloid with Existing Memory Tiering Systems (Colloid, SOSP'24)|Section 4]] 에서도 설명하겠지만, 이런 periodical 한 방식으로 운영되지 않는 tiered memory system (가령 hint fault 를 사용하는 TPP 등) 에도 통합할 수 있다.
...
- 용어정리를 좀 해보자.
	...
	- *Memory Interconnect Contention* 은 다음의 두 상황에 의해 *Loaded Latency* 가 *Unloaded Latency* 보다 커지는 것을 의미한다.
		1) 첫번째는 BW saturation 이다: CPU-memory 사이에 있는 어느 interconnect 이든 그놈의 BW 가 saturate 되어서 memory request 가 queueing 될때 발생할 수 있다.
		...
		2) 두번째는 메모리의 내부 특성에 의한 serialization point 때문이다: BW 가 saturate 되지 않아도, 메모리의 내부 구조때문에 contention 이 발생할 수도 있다.
			- 가령 DRAM 은 여러개의 bank 들로 구성되어 있는데,
			- [[Locality (Replacement)|Locality]] 가 부족해서 caching 이 되지 않는 request 들이 일부의 bank 에 몰리게 되면 당연히 동일 bank 에 대한 request 는 serial 하게 처리되므로 이놈들이 memory controller 안에서 queueing 된다.
			- 그래서 latency 가 늘어나게 되는 것.
...
- 그럼 "Balancing access latencies" 는 다음과 같이 정의할 수 있다.
	1) 만약 $L_{D} \lt L_{A}$ 라면, $p$ 를 증가시키는 방향으로 page placement 가 이루어져야 한다.
		- 당연히 default tier 의 latency 가 작다는 의미이므로 default tier 에 hot page 를 더 많이 배치해 $p$ 를 늘려야 한다 (default tier 에 더 많이 접근되도록 해야 한다).
	2) 만약 $L_{D} \gt L_{A}$ 라면, $p$ 를 감소시키는 방향으로 page placement 가 이루어져야 한다.
		- 마찬가지로, 이것은 default tier 의 latency 가 크다는 의미이므로 alternate tier 에 hot page 를 더 많이 배치해 $p$ 를 줄여야 한다 (default tier 에 더 적게 접근되도록 해야 한다).
	3) 만약 $L_{D} = L_{A}$ 라면, optimal 한 상태이므로 추가적인 page placement 가 필요하지 않다.
- 그럼 "Balancing access latencies" 이 왜 application 의 성능을 최대로 뽑아내는 것에 중요한지, 그리고 이를 위해서는 왜 각 상황에서 $p$ 를 저렇게 조정해야 하는지를 생각해 보자.
	...
	- 우선, CPU 가 보낼 수 있는 최대 memory request 의 개수는 (모든 CPU cache 에서 miss 된 것들이 memory request 로 날라가기 때문에) CPU 의 [[Line Fill Buffer (Arch)|Line Fill Buffer]] 에 의해 한정되게 된다.
	- Core 당 memory access throughput 의 평균은 $N \times 64 \over L$ 이고, 이때 $N$ 은 core 당 한번에 보낼 수 있는 memory request 의 최대치이므로 이 값은 최대치가 정해져 있다.
		- 여기서 64 는 cacheline 단위로 memory request 가 날라가기 때문에 cacheline size 인 64byte 를 의미한다.
	- 따라서 memory throughput 은 average latency 와 반비례하는 방향으로 직접적으로 연결되어 있고, 따라서 application 의 성능도 average latency 에 직접적으로 영향을 받게 된다.
		...
		- 마지막으로, $L_{D} = L_{A}$ 라면 $p$ 에 0이 곱해지는 꼴이므로 $p$ 를 바꾸는 것이 average latency 에 영향을 주지 않는다. 이 지점이 바로 average latency 가 최소가 되는 지점이다.
- "Balancing access latencies" 의 원칙은 다음의 metric 들이 자연스레 녹아들어 unified approach 를 가능하게 만들어 준다.
	1) 이 원칙에는 *Unloaded Latency* 가 자연스럽게 반영된다. 왜냐면 default 와 alternate 간의 *Unloaded Latency* 차이가 크다면 default tier 에 *Memory Interconnect Contention* 이 발생해도 여전히 default tier 의 latency 가 작을 것이고, 따라서 default tier 에 page 가 배치될 것이기 때문이다.
	...
	2) 또한, 각 tier 의 maximum BW 또한 고려된다. 왜냐면 BW 가 높다면 BW saturation 이 잘 발생하지 않아 *Loaded Latency* 가 작게 나올 것이기 때문이다.
		- 즉, maximum BW 는 *Loaded Latency* 에 영향을 주는 요소이기 때문에 이것 또한 반영된다는 것.
	3) 마지막으로, 위에서 말한 controller contention 도 반영이 된다. 즉, memory 내부 구조 등의 영향으로 BW 가 saturate 되지 않은 상황에서도 memory interconnect contention 이 발생하는 것 또한 반영된다. 이 Controller contention 이 발생하면 마찬가지로 *Loaded Latency* 가 증가하기 때문.
...
- 우선 memory 의 throughput 을 결정하는 request 는 read request 이고, 따라서 *Colloid* 에서는 read latency 만을 측정한다고 한다.
	...
	- 즉, write performance 역시 read performance 에 영향을 받게 된다.
	- Cache 에 있는 데이터가 memory 에 쓰이는 것은 write back 될때로, cacheline 이라는 batch 단위로 asynchronous 하게 처리되기 때문에 read 에 비해서는 빈도가 낮다 [^write-back].
- Latency 를 측정하는 방법으로는, [[Caching Home Agent, CHA (Intel Cache Arch)|CHA]] 를 이용한다.
	...
	- 이런 CHA 에서는 queue occupancy counter (`TOR_OCCUPANCY`)와 request arrival rate 를 request type (read, write) 와 tier 별로 알려주는 counter (`TOR_INSERTS`) 가 존재하고, *Colloid* 에서는 이것을 사용하게 된다.
...
- 그럼 여기서 빠진게 하나 있는데, 바로 CPU-CHA 간의 latency 이다. 근데 이건 너무 작아서 무시해도 된다고 한다.
	...
	- 또한 memory interconnect contention 상황에서는 CHA-memory latency 가 더욱 커지므로 이 CPU-CHA latency 는 더욱 작아지게 된다.
- 참고로 각 occupancy 와 arrival rate 는 *Exponentially Weighted Moving Averaging* (*EWMA*) 이라는 것으로 보정하여 noise 를 줄였다고 한다 [^ewma].
	- 물론 EWMA 때문에 latency 계산에 걸리는 시간이 조금 더 추가되긴 하지만, 이것은 system stability 와 trade-off 관계로서 감당하기로 했다고 한다.
...
- 다음에는 어떤 page 를 migrate 할지를 정하는데, 이것은 다음의 세 단계로 이루어진다.
	1) 우선 $p$ 를 얼마나 조정할지를 결정한다. 이것은 $ComputeShift()$ 를 통해 결정되며, 기호는 $\Delta p$ 이다.
	...
	2) 다음으로는 migration limit 을 구하는 것이다.
	...
	3) 마지막으로는 실제 page migration system (HeMem 등) 으로 page migration 을 수행하게 된다.
		- 여기서는 다음의 두가지 제약조건이 있다.
			1. 옮기려는 page 의 access probability 의 총합이 $\Delta p$ 보다 작거나 같아야 한다.
				- 당연히 $\Delta p$ 만큼을 옮기는 것이 목표이므로 $\Delta p$ 와 같으면 좋겠지만, 만약 모든 page 를 전부 옮겨도 해당 수치를 못맞추는 경우에는 어쩔 수 없이 옮길 수 있는 만큼만 옮기게 된다.
			2. 옮기려는 page 의 총 크기가 migration limit 및 destination tier 의 여유공간을 넘으면 안된다.
...
[^write-back]: 이건 주인장의 추측이다. 논문에 이 빈도와 관련된 언급은 없음.
