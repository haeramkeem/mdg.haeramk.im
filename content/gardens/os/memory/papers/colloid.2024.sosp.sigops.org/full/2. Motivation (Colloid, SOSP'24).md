...
- 본 [[2. Motivation (Colloid, SOSP'24)|Section]] 에서는 SOTA 인 [[(논문) HeMem - Scalable Tiered Memory Management for Big Data Applications and Real NVM (SOSP'21)|HeMem]], [[(논문) MEMTIS - Efficient Memory Tiering with Dynamic Page Classification and Page Size Determination|MEMTIS]], 그리고 [[(논문) TPP - Transparent Page Placement for CXL-Enabled Tiered-Memory|TPP]] 를 분석하며 다음에 대한 demonstration 을 한다:
	- *Memory Interconnect Contention* 상황에서는, default tier 의 latency 는 unloaded 에 비해 5배까지 치솟을 수 있다는 것을 보인다.
		- 그리고 이것은 alternate tier 의 unloaded latency 에 비해 2.4배정도 더 크다.
		- 또한 CXL 의 HW spec 에 따른 latency 와 비교하면 이것은 2.5 배정도 더 큰 수치라고 한다.
...
- HW 세팅은:
	- Socket 두개에 각각 Intel Xeon Platinum 8362 (32C, 1.25MB L2, 48MB [[L1, L2, L3 Cache (Arch)|LLC]]) 가 들어있다.
		- 각 socket 에는 8개의 3200MHz DDR4 channel 이 있고 각 채널에는 DIMM memory 가 하나씩 달려있다고 한다.
		- 그래서 이론상의 최대 대역폭은 205GB/s 라고 한다 [^mem-bw].
...
- Workload 세팅은:
	...
	- 90% 확률로 hot set 를 선택하고 나머지 10% 확률로 전체 set 중에 하나를 선택한다고 한다.
...
- Workload 를 위한 core 할당은:
	...
	- 남은 15개 (16-30) 에는 *Antagonist* 를 돌린다.
		...
		- 구체적으로는, 이놈들은 default tier 에 pinning 된 500MB 의 데이터를 sequential 하게 1:1 비율로 read, write 한다.
...
- 그럼 저 antagonist 에 대해 좀 더 알아보면
	...
	- 실험에서는 이 antagonist 의 수를 조절해가면서 steady state 일때의 throughput 을 측정한다.
		...
		- 저 antagonist 의 개수 (0, 5, 10, 15 개) 를 그래프에서는 $0\times$, $1\times$, $2\times$, $3\times$ 으로 표기하고, 각각에 대해 antagonist 가 차지하는 대역폭은 전체의 0%, 51%, 61%, 70% 라고 한다 (이건 antagonist 만 실행시켰을 때의 대역폭을 측정한 것이다).
		- 이 실험에서는 steady state 에 대해서만 실험했고 dynamic state 에 대해서는 [[5. Evaluation (Colloid, SOSP'24)|section 5]] 에서 다룬다.
- 마지막으로, optimal throughput (best case) 는 다음과 같이 측정했다.
	...
	- 즉, hot page 를 처음에는 모두 (100%) default tier 에 넣어보고, 다음에는 90% 만 default tier 에 넣어보고, 하는 작업을 반복하며 가장 throughput 이 잘 나오는 비율을 찾았다는 것.
		- 4개의 setting 에 대해 10개 (0%, 10%, ..., 100%) 의 경우의 수이므로 총 40개의 조건들을 직접 해보며 찾았다는 소리이다.
		- 그리고 default tier 의 남는 공간은 cold page 를 임의로 골라 채웠다.
...
- 일단 $0\times$ 일 때를 보면
	...
	- 근데 MEMTIS 는 10.1% 의 감소를 보이며 비교적 큰 성능 하락이 있었는데, 이것은 MEMTIS 의 hugepage coalescing 에 문제가 있기 때문이라고 한다.
		...
		- 본 실험에서 2Mi page 를 4Ki page 로 쪼개는 것이 별로 도움이 안됨에도 불구하고 이렇게 쪼개는 동작을 보여주었기 때문이라고 한다.
		- 좀 더 구체적으로 말하면, 2Mi page 를 4Ki page 로 쪼갠 뒤 다시 합칠 때 virtual address space 전체를 을 scan 하면서 수행하기에 너무 오래걸려 steady state 가 되기 전에 이 작업이 다 끝나지 않아 이런 overhead 가 발생한다고 한다.
...
- 그럼 이때 각 tier 의 latency 를 살펴보자.
	- 이 latency 는 Intel 의 [[Caching Home Agent, CHA (Intel Cache Arch)|CHA]] 기능을 이용해 측정했다고 한다.
...
- 이 이유는 memory controller 에서의 queue occupancy 때문이고, 이것때문에 memory BW 가 saturate 되지 않는 상황에서도 위와 같은 latency increase 가 발생하게 된다. 이에 대해서는 [[3. Colloid (Colloid, SOSP'24)|Section 3]] 에서 더 자세히 알아보자.
...
- 그럼 각 tier 가 차지하는 BW 를 살펴보자.
	...
	- 즉, default tier 의 BW 가 높으면 default tier 에 hot page 가 많다고 추론할 수 있는 셈.
	- 이것은 Intel 의 [[Memory Bandwidth Monitoring, MBM (Intel Memory Arch)|MBM]] 기능을 이용해 알아냈다고 한다.
...
- 따라서 latency 에 반비례해서 throughput 줄어든다는 것을 알 수 있다.
- 근데 [[#2.2.1. Under memory interconnect contention, default tier access latency can exceed that of alternate tier.|Section 2.2.1]] 에서의 그래프를 보면 $0\times$ 에서 $3\times$ 로 바뀜에 따라 latency 는 3.5 배 늘어나므로 이론적인 throughput drop 또한 3.5 배일 것이다.
- 이것은 [[#2.2.0. Result|Section 2.2.0]] 의 그래프를 보면 알 수 있다: 이 그래프를 보면, throughput 은 HeMem, TPP, MEMTIS 각각 3.42배, 3.39배, 3.29배 작아진다고 한다 [^throughput].

[^mem-bw]: 왜요??
[^throughput]: 사실 이부분은 좀 안맞는거 아닌가? 2.2.0 에서의 그래프를 보면 MEMTIS 의 경우에 70 에서 20으로 곤두박질쳤으니 3.5 배이고 나머지는 80 가까이에서 20 가까이로 떨어졌으니 4배에 가까운 감소인거 아닌가?